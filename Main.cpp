...
void InitData() {
	for (int i = 0; i < PROCESSNUMBERMAX; i++) {
		...
		nt[i].leavebTime;
		nt[i].NormalizedTT = 0;
		nt[i].ResponseRatio = 0;
		nt[i].totalTime = 0;
		nt[i].trunaroundTime = 0;
		nt[i].waitingTime = 0;
		dopyo[i].id = 0;
	}
	for (int i = 0; i < 100; i++) {
...
void SRTN(int n) {
	...
	while (1) {
		...
		sec++;
		printf("%d ", nt[p_id].leavebTime);
		nt[p_id].leavebTime--;

		//현재 프로세스 종료
		if (nt[p_id].leavebTime == 0) {
			//각 시간 구하기
			nt[p_id].endTime = sec;
		...
		for (int i = 0; i < n; i++) {
			//기존 프로세스를 계속한다.
			if ((sec==nt[i].arrivalTime)&& (nt[p_id].leavebTime < nt[i].leavebTime)) {
				enqueue(&ready, nt[i]);
				count++;

			}
			else if ((sec == nt[i].arrivalTime) && (nt[p_id].leavebTime > nt[i].leavebTime)) {
				enqueue(&ready, nt[p_id]);
				count++;
				p_id = i;
		...
		for (int i = 0; i < count; i++)
		{
			for (int j = 0; j < count - 1; j++)
			{
				if (s_temp[j].leavebTime > s_temp[j + 1].leavebTime)
				{
					temp2 = s_temp[j + 1];
					s_temp[j + 1] = s_temp[j];
					s_temp[j] = temp2;
		...
		//printf("%d", count);
		//기존 프로세스를 enqueue하고 도착한 프로세스를 실행한다. 
